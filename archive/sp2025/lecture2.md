...
(1)emacs是另一个将快捷键使用到炉火纯青的编辑器，和vim不同，emacs追求极致的扩展性，甚至为了扩展性它的本体就要占据117MB的空间。如果有兴趣可以尝试一下emacs。在emacs中自带一个教程(emacs tutorial)，可以进行参考并学习。  
(2)在一位核善的老学长的提醒下加入了这道题：试试vscode，然后根据自己喜欢的编程语言找找插件试着用一用  
(3)尝试触发这个程序的缓冲区溢出漏洞：  
```C
#include <stdio.h>
char input[5];
...
(4)展开示例的makefile，和你输入make之后产生的命令比对一下看看你的展开是不是正确的  
